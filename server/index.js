// IMPORTANT: Tracing must be loaded via --import flag in package.json
// due to ESM vs CJS compatibility issues with OpenTelemetry auto-instrumentation
...
import { metricsMiddleware, register, errorTotal, courseRequestsTotal } from './metrics.js';
...
async function parseUdemyCourses(html) {
  ...
  try {
    ...
    logger.error('Error parsing Udemy courses', { error: error.message });
  }
...
async function parseCourseraCourses(html) {
  ...
  try {
    ...
    logger.error('Error parsing Coursera courses', { error: error.message });
  }
...
async function handleExternalCourses(req, res) {
  try {
    logger.info('Fetching external courses');
    courseRequestsTotal.inc({ endpoint: 'external' });
    ...
    logger.error('Error in external courses endpoint', { error: error.message, stack: error.stack });
    errorTotal.inc({ type: 'api_error', service: 'courses' });
    res.status(500).json({
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      metrics: '/metrics',
      courses: '/api/courses/external',
      alternativeCourses: '/courses/external'
    },
...
app.get('/api/test-error', (req, res) => {
  ...
  errorTotal.inc({ type: 'test_error', service: 'api' });
...
app.use((err, req, res, next) => {
  ...
  errorTotal.inc({ type: 'server_error', service: 'api' });
...
app.listen(PORT, () => {
  ...
  logger.info(`Liveness probe: http://localhost:${PORT}/health/live`);
  logger.info(`Readiness probe: http://localhost:${PORT}/health/ready`);
  logger.info(`Metrics: http://localhost:${PORT}/metrics`);
  logger.info(`Courses API: http://localhost:${PORT}/api/courses/external`);
}).on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    logger.error(`Port ${PORT} is already in use. Please stop the other server or change the PORT.`);
  } else {
